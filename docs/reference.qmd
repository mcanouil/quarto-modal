...
::: {.callout-caution}
`centred` is spelled the British way, and only that spelling reaches the dialog.
`centered` written on its own is read as an ordinary attribute and does nothing, with no warning; written alongside `centred` it warns and is discarded.
:::
